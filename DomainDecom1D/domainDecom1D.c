...
int main(int argc, char *argv[]){
    ...
    if (rank != 0) {
        ...
        MPI_Recv(&f[0], 1, MPI_DOUBLE, (rank - 1) % size, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    else {
        MPI_Recv(&f[0], 1, MPI_DOUBLE, size - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Send(&f[nxn_loc - 3], 1, MPI_DOUBLE, (rank + 1) % size, 0, MPI_COMM_WORLD);
    }

    if (rank != 0) {
        MPI_Send(&f[2], 1, MPI_DOUBLE, (rank - 1) % size, 1, MPI_COMM_WORLD);
        MPI_Recv(&f[nxn_loc - 1], 1, MPI_DOUBLE, (rank + 1) % size, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    else {
        MPI_Recv(&f[nxn_loc - 1], 1, MPI_DOUBLE, (rank + 1) % size, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Send(&f[2], 1, MPI_DOUBLE, size - 1, 1, MPI_COMM_WORLD);
    }
    ...
    for(i = 1; i < (nxn_loc - 1); i++) {
        if(abs(dfdx[i] - cos(L_loc*rank + (i-1) * dx)) >= 0.001)
            printf("Error on rank: %d, i=%d, %f ~= %f?\n", rank, i, dfdx[i], cos(L_loc*rank + (i-1) * dx));
    }
    printf("Beg node %d: %f %f\n", rank, f[0], f[2]);
    printf("End node %d: %f %f\n", rank, f[nxn_loc - 3], f[nxn_loc - 1]);
